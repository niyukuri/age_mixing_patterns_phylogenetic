---
title: "Inferring age mixing patterns from phylogenetic trees: Missing at Random (women = 50%)"
...
Inferring age mixing patterns from phylogenetic trees when missingess sequences are Missing Completly at Random, and we have at most 50% of women in all age groups.
...
ARMSE <- function(v1=v1, v2=v2) { # v1 = y_i and v2= x_i
  d <- data.frame(v1,v2)
  r <- na.omit(d)
  y1 <- r[,1]
  y2 <- r[,2]
...
RMSE <- function(error) { # vector y_i - x_i
  error <- as.numeric(na.omit(error))
  rmse <- sqrt(mean(error^2)) 
  return(rmse)}


MAE <- function(error) { # vector y_i - x_i
  error <- as.numeric(na.omit(error))
  mae <- mean(abs(error)) 
  return(mae)
}
...
MRE <- function(error) {  # vector y_i - x_i
  error <- as.numeric(na.omit(error))
  mae <- mean(error) 
  return(mae)
}
...
An HIV epidemic was simulated in an age and gender structured population. With an initial population of 10000 men and 10000 women, the simulation time was 40 years, and the infection was introduced in the population at the 10th year among randomly selected 10 individuals within 20 and 50 of age range. During simulation, demographic events like birth, and death were considered together with sexual partnerships. Refering to real world history of HIV, mainly ART treatment, we gradually allow interventions based on CD4 counts. On calendar time the simulation started in 1977, and the infection was introduced in 1987 for 30 years in 2017. With same parameter combination, we performed 2800 simulations, and the statistics presented below will be minimum, median, mean, and maximum values of all 2800 simulations. 
...
Incidence within a time interval of 35 - 40 years of running time. 
...
Age mixing patterns is defined as population-level patterns for how people choose partners with regards to age. If we consider past 5 years backward from simulation time 40, we have the following statistics of age mixing patterns in partnership.
...
Note that we took the age of a man and substract the age of his partner (woman), hence we have suffix ".male" on these parameters
...
age.mix.stats <- round(age.mix.stats, digits = 3)
...
Per each simulation for the existing 2800, we computed transmission cluster size statistics: mean, median, and standard deviation to have an idea on their size distribution. Here below, we present the median values of these statistics for overall 2800 simulations in different sampling coverage.
...
Since the simlation was performed 2800 times, the number of pairings presented in the tables below are median values.
...
d_f$age_groups <- c("F.15.24.M.15.24", "F.15.24.M.25.39", "F.15.24.M.40.49", "F.25.39.M.25.39", "F.25.39.M.40.49", # females_males
                 "M.15.24.F.15.24", "M.25.39.F.15.24", "M.25.39.F.25.39", "M.40.49.F.15.24", "M.40.49.F.25.39") # males_females


# x <- c(rep(c("15_24 & 15_24", "15_24 & 25_39", "15_24 & 40_49", "25_39 & 25_39", "25_39 & 40_49"), 2))

x <- c("F.15.24.M.15.24", "F.15.24.M.25.39", "F.15.24.M.40.49", "F.25.39.M.25.39", "F.25.39.M.40.49", # females_males
                 "M.15.24.F.15.24", "M.25.39.F.15.24", "M.25.39.F.25.39", "M.40.49.F.15.24", "M.40.49.F.25.39") # males_females
...
d_m <- dplyr::filter(d_f, d_f$f_m=="Males_Females")
d_f <- dplyr::filter(d_f, d_f$f_m=="Females_Males")

p1 <- ggplot(d_f, aes(x=x, y=med, colour=age_groups)) + 
...
p2 <- ggplot(d_m,aes(x=x, y=med, colour=age_groups)) + 
  geom_errorbar(aes(ymin=lower.Q1, ymax=upper.Q3), width=.5) +
  geom_line(size=.5) +
  geom_point(size=1) + 
  theme(legend.position="top")+
...
# d_f$age_group <- c(rep(c("15_24 & 15_24", "15_24 & 25_39", "15_24 & 40_49", "25_39 & 25_39", "25_39 & 40_49"), 2))


d_f$age_group <- c("F.15.24.M.15.24", "F.15.24.M.25.39", "F.15.24.M.40.49", "F.25.39.M.25.39", "F.25.39.M.40.49", # females_males
                 "M.15.24.F.15.24", "M.25.39.F.15.24", "M.25.39.F.25.39", "M.40.49.F.15.24", "M.40.49.F.25.39") # males_females
...
p1 <- ggplot(v_f, aes(x=cov, y=val, colour= age_groups, group = age_groups)) + 
  geom_line(size=1) +
  geom_point() +
  # facet_grid(. ~ f_m) + 
  theme(legend.position="bottom")+
...
p2 <- ggplot(v_m, aes(x=cov, y=val, colour= age_groups, group = age_groups)) + 
  geom_line(size=1) +
  geom_point() +
  # facet_grid(. ~ f_m) + 
  theme(legend.position="bottom")+
...
names_age_groups <- c("F.15.24.M.15.24", "F.15.24.M.25.39", "F.15.24.M.40.49", "F.25.39.M.25.39", "F.25.39.M.40.49", # females_males
                 "M.15.24.F.15.24", "M.25.39.F.15.24", "M.25.39.F.25.39", "M.40.49.F.15.24", "M.40.49.F.25.39") # males_females

names_age_groups_x <- c("F.15.25.M.15.25", "F.15.25.M.25.40", "F.15.25.M.40.50", "F.25.40.M.25.40", "F.25.40.M.40.50", # females_males
                 "M.15.25.F.15.25", "M.25.40.F.15.25", "M.25.40.F.25.40", "M.40.50.F.15.25", "M.40.50.F.25.40") # males_females

age_group_all <- vector()

for(i in 1:nrow(newdata)){
  age_group_i <- newdata$param[i] 
  for(j in 1:length(names_age_groups_x)){
    if(age_group_i==names_age_groups_x[j]){
      age_group_all <- c(age_group_all, names_age_groups[j])
    }
  }
}


newdata$age_groups <- age_group_all
...
newdata_f <- dplyr::filter(newdata, newdata$f_m=="Females_Males")

newdata_m <- dplyr::filter(newdata, newdata$f_m=="Males_Females")


p11 <- ggplot(newdata_f, aes(x=cov, y=val, colour= age_groups, group = age_groups)) + 
  geom_line(size=1) +
  geom_point() +
  # facet_grid(. ~ f_m) + 
  theme(legend.position="bottom")+
...
p22 <- ggplot(newdata_m, aes(x=cov, y=val, colour= age_groups, group = age_groups)) + 
  geom_line(size=1) +
  geom_point() +
  # facet_grid(. ~ f_m) + 
  theme(legend.position="bottom")+
...
grid.arrange(p1, p2, p11, p22, ncol=2, nrow=2)


ggsave(filename = "Plot_c_9_10_proportion_pairings_and_error_at_35_95_Coverage.pdf",
       grid.arrange(p1, p2, p11, p22, ncol=2, nrow=2),
       path = "/home/david/age_mixing_patterns_phylogenetic/results/MAR_c",
...
AD.stats <- round(AD.stats, digits = 3)
...
df_all$age_groups <- c(rep("15_24", 14), rep("25_39", 14), rep("40_49", 14), rep("15_24", 14), rep("25_39", 14), rep("40_49", 14))
...
plot.mean.men.women.AD.df <- ggplot(df_all, aes(x=cov, y=F, colour=age_groups, group=Parameter)) + 
  geom_line(size=1) +
  geom_point() +
  facet_grid(.~f_m)+
  theme(legend.position="bottom")+
...
df_all_med$age_groups <- c(rep("15_24", 14), rep("25_39", 14), rep("40_49", 14), rep("15_24", 14), rep("25_39", 14), rep("40_49", 14))

plot.med.men.women.AD.df <- ggplot(df_all_med, aes(x=cov, y=F, colour=age_groups, group=Parameter)) + 
  geom_line(size=1) +
  geom_point() +
  facet_grid(.~f_m)+
  theme(legend.position="bottom")+
...
df_all_sd$age_groups <- c(rep("15_24", 14), rep("25_39", 14), rep("40_49", 14), rep("15_24", 14), rep("25_39", 14), rep("40_49", 14))
...
plot.sd.men.women.AD.df <- ggplot(df_all_sd, aes(x=cov, y=F, colour=age_groups, group=Parameter)) + 
  geom_line(size=1)+
  geom_point() +
  facet_grid(.~f_m)+
  theme(legend.position="bottom")+
...
newdata_mean$age_groups <- c(rep("25_24", nrow(RMSE.error.infer.clust.cov.100.men.15.25.AD.mean)),
                             rep("25_39", nrow(RMSE.error.infer.clust.cov.100.men.25.40.AD.mean)),
                             rep("40_49", nrow(RMSE.error.infer.clust.cov.100.men.40.50.AD.mean)),
                             
                             rep("25_24", nrow(RMSE.error.infer.clust.cov.100.women.15.25.AD.mean)),
                             rep("25_39", nrow(RMSE.error.infer.clust.cov.100.women.25.40.AD.mean)),
                             rep("40_49", nrow(RMSE.error.infer.clust.cov.100.women.40.50.AD.mean)))
...
newdata_sd$age_groups <- c(rep("25_24", nrow(RMSE.error.infer.clust.cov.100.men.15.25.AD.sd)),
                             rep("25_39", nrow(RMSE.error.infer.clust.cov.100.men.25.40.AD.sd)),
                             rep("40_49", nrow(RMSE.error.infer.clust.cov.100.men.40.50.AD.sd)),
                             
                             rep("25_24", nrow(RMSE.error.infer.clust.cov.100.women.15.25.AD.sd)),
                             rep("25_39", nrow(RMSE.error.infer.clust.cov.100.women.25.40.AD.sd)),
                             rep("40_49", nrow(RMSE.error.infer.clust.cov.100.women.40.50.AD.sd)))
...
ggsave(filename = "Plot_c_11_15_Mean_Age_Difference_and_Error_at_35_95_Coverage.pdf",
       grid.arrange(plot.mean.men.women.AD.df, plot.mean.AD_seq_cov_errors_100, nrow=2),
       path = "/home/david/age_mixing_patterns_phylogenetic/results/MAR_c",
       width = 40, height = 25, units = "cm")



ggsave(filename = "Plot_c_13_16_SD_Age_Difference_and_Error_at_35_95_Coverage.pdf",
       grid.arrange(plot.sd.men.women.AD.df, plot.sd.AD_seq_cov_errors_100, nrow=2),
       path = "/home/david/age_mixing_patterns_phylogenetic/results/MAR_c",
       width = 40, height = 25, units = "cm")
